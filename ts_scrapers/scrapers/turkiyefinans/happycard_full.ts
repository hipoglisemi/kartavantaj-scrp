/**
 * @file happycard_full.ts
 * @status 🟢 ACTIVE
 * @last_verified 2026-01-19
 * @description Scraper for Türkiye Finans Happy Card campaigns - processes all 30 campaigns from extracted URLs
 */
...
// Read campaign URLs from file (extracted from browser)
const CAMPAIGN_URLS_FILE = '/tmp/happycard_campaign_links.txt';

const sleep = (ms: number) => new Promise(resolve => setTimeout(resolve, ms));

async function runHappyCardFullScraper() {
    console.log('🚀 Starting Türkiye Finans Happy Card Full Scraper...');
    ...
    try {
        ...
        for (const fullUrl of urlsToProcess) {
            ...
            try {
                ...
                // Note: h1 contains generic "Kampanyalar", actual title is in h2 or title tag
                let title = $detail('.campaign-detail h2').first().text().trim();

                // If h2 is empty, try title tag
                if (!title || title === 'Kampanyalar') {
                    const titleTag = $detail('title').text().trim();
                    // Remove common suffixes from title tag
                    title = titleTag
                        .replace('- Happy Card', '')
                        .replace('Türkiye Finans Happy Kredi Kartları Kampanyalar', '')
                        .replace('Türkiye Finans Happy Kredi Kartları', '')
                        .replace(/\s+-\s+Happy Card$/i, '')
                        .trim();
                }
        ...
        console.log(`\n✅ Happy Card Full Scraper Finished. Processed ${processedCount} campaigns.`);
