...
class YapikrediCrystalScraper:
    ...
    def __init__(self):
        ...
        with get_db_session() as db:
            ...
            card = db.query(Card).filter(Card.slug == "crystal", Card.bank_id == self.bank.id).first()
            if not card:
                print(f"Creating card: {self.CARD_NAME}")
                card = Card(name=self.CARD_NAME, bank_id=self.bank.id, slug="crystal", card_type="credit", logo_url="/logos/cards/yapikredicrystal.png", is_active=True)
                db.add(card)
                db.commit()
                db.refresh(card)
            self.card = card
