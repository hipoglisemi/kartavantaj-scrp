...
class YapikrediCrystalScraper:
    ...
    def _get_or_create_card(self) -> Card:
        card = self.db.query(Card).filter(Card.slug == "crystal", Card.bank_id == self.bank.id).first()
        if not card:
            ...
            card = Card(
                ...
                slug="crystal",
                card_type="credit",
                logo_url="/logos/cards/yapikredicrystal.png",
                is_active=True
            )
            self.db.add(card)
            self.db.commit()
             ...
             if not card.logo_url:
                card.logo_url = "/logos/cards/yapikredicrystal.png"
                self.db.commit()
        ...
        try:
            ...
            if tracking_url:
                try:
                    ...
                    path = tracking_url.split('crystalcard.com.tr')[-1] # /kampanyalar/slug
    ...
    def run(self):
        ...
        try:
            ...
            log_scraper_execution(
                 db=self.db,
                 scraper_name="yapikredi-crystal",
                 status=status,
